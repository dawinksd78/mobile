<!DOCTYPE Html>
<html lang="Ko">
<head>
<meta charset="utf-8" />
<meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
<title>다윈중개 모바일 - 평가하기</title>
<meta name="viewport" content="width=device-width, user-scalable=no, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0" />
<meta name="format-detection" content="telephone=no, address=no, email=no" />
<meta name="apple-mobile-web-app-capable" content="yes">
<link rel="stylesheet" type="text/css" href="/css/m_all.css">
<link rel="stylesheet" type="text/css" href="/css/notosanskr.css">
<link rel="stylesheet" type="text/css" href="/css/nanumsquare.css">
<script type="text/javascript" src="/js/jquery-1.9.1.min.js"></script>
</head>

<body>
<div id="dawinWrap" class="nomem_survey">
    <section id="container">
        <div class="sub_container">
            <div class="cont_wrap public_cont02 rate">
              	<div class="subjwrap">
                	<h2 class="subj_tit">
                		<span class="m_tit">다윈중개 중개평가하기</span>
                		<p class="m_exp">다윈중개에서 거래해 주셔서 감사합니다.<br>서비스 향상과 중개사 피드백을 위해 아래 간단한 설문을 부탁드립니다.<br>고객님께서 남겨주시는 소중한 후기는 다른 고객님들께 유익한 정보가 됩니다.</p>
                		<div class="star_score">
                			<span class="st_off"><span class="st_on" style="width:91.2%"></span></span>
                    		<output for="star_input"><b>4</b>/5</output>
                		</div>
                	</h2>
              	</div>
                
                <div class="cont">
                    <div class="rt_wrap">
                        <div class="rt_items">
                        	<span class="p_area">1. 친절하고 신뢰를 드렸나요?</span>
                            <span class="e_area star_input">
                            	<span class="input">
                                    <input type="radio" name="star_input" value="1" id="p1">
                                    <label for="p1" class="lbl01">1</label>
                                    <input type="radio" name="star_input" value="2" id="p2">
                                    <label for="p2" class="lbl02">2</label>
                                    <input type="radio" name="star_input" value="3" id="p3">
                                    <label for="p3" class="lbl03">3</label>
                                    <input type="radio" name="star_input" value="4" id="p4">
                                    <label for="p4" class="lbl04">4</label>
                                    <input type="radio" name="star_input" value="5" id="p5">
                                    <label for="p5" class="lbl05">5</label>
                                </span>
                                <output for="star_input"><b>3</b>/5</output>
							</span>
                        </div>
                        <div class="rt_items">
                        	<span class="p_area">2. 전문적 지식과 경험을 가지고 잘 설명해 주었나요?</span>
                            <span class="e_area star_input">
                                <span class="input">
                                    <input type="radio" name="star_input" value="1" id="p6">
                                    <label for="p6" class="lbl01">1</label>
                                    <input type="radio" name="star_input" value="2" id="p7">
                                    <label for="p7" class="lbl02">2</label>
                                    <input type="radio" name="star_input" value="3" id="p8">
                                    <label for="p8" class="lbl03">3</label>
                                    <input type="radio" name="star_input" value="4" id="p9">
                                    <label for="p9" class="lbl04">4</label>
                                    <input type="radio" name="star_input" value="5" id="p10">
                                    <label for="p10" class="lbl05">5</label>
                                </span>
                                <output for="star_input"><b>3</b>/5</output>
                            </span>
                        </div>
                        <div class="rt_items">
                        	<span class="p_area">3. 문의하신 내용에 대해 빠른 응대를 드렸나요?</span>
                            <span class="e_area star_input">
                            	<span class="input">
                                    <input type="radio" name="star_input" value="1" id="p11">
                                    <label for="p11" class="lbl01">1</label>
                                    <input type="radio" name="star_input" value="2" id="p12">
                                    <label for="p12" class="lbl02">2</label>
                                    <input type="radio" name="star_input" value="3" id="p13">
                                    <label for="p13" class="lbl03">3</label>
                                    <input type="radio" name="star_input" value="4" id="p14">
                                    <label for="p14" class="lbl04">4</label>
                                    <input type="radio" name="star_input" value="5" id="p15">
                                    <label for="p15" class="lbl05">5</label>
                                </span>
                                <output for="star_input"><b>3</b>/5</output>
                            </span>
                        </div>
                        <div class="rt_items">
                        	<span class="p_area">4. 거래 당사자간 의견을 잘 조율해 주셨나요?</span>
                            <span class="e_area star_input">
                            	<span class="input">
                                    <input type="radio" name="star_input" value="1" id="p16">
                                    <label for="p16" class="lbl01">1</label>
                                    <input type="radio" name="star_input" value="2" id="p17">
                                    <label for="p17" class="lbl02">2</label>
                                    <input type="radio" name="star_input" value="3" id="p18">
                                    <label for="p18" class="lbl03">3</label>
                                    <input type="radio" name="star_input" value="4" id="p19">
                                    <label for="p19" class="lbl04">4</label>
                                    <input type="radio" name="star_input" value="5" id="p20">
                                    <label for="p20" class="lbl05">5</label>
                                </span>
                                <output for="star_input"><b>3</b>/5</output>
                            </span>
                        </div>
                        <div class="rt_items">
                        	<span class="p_area">5. 약정된 중개수수료 이외의 다른 금품 등을 요구 받은적이 있으신가요?</span>
                            <div class="radio_box">
                                <div class="radio">
                                    <label for="rd01">
                                        <input type="radio" name="rd" id="rd01" value="Y">
                                        <i></i><strong>있음</strong>
                                    </label>
                                </div>
                                <div class="radio">
                                    <label for="rd02">
                                        <input type="radio" name="rd" id="rd02" value="N">
                                        <i></i><strong>없음</strong>
                                    </label>
                                </div>
                            </div>
                        </div>
                        <div class="rt_items"> 
                          	<span class="p_area">6. 중개사에게 하고싶은 말이나 평가하는 이유를 작성해주세요.</span>
                          	<textarea class="txtarea" placeholder="내용을 입력해주세요."></textarea>
                        </div>
                    </div>
                </div>
            </div>
            
            <div class="btn_area bot_btn">
                <button type="button" class="btn_type03">중개평가완료</button>
                <!-- 사유 선택 후 경우 on 클래스 추가 --> 
            </div>
        </div>
    </section>
    
    <!-- 중개평가완료 시 팝업 -->
    <div class="mask" style="display:none"></div>
    <div class="lyr lyrpop01" style="display:none">
        <div class="lyr_inner">
            <p class="cont"><b>중개평가</b>가 완료되었습니다.<br>감사합니다.</p>
        </div>
        <div class="btn">
            <button type="button" class="btn_type02">확인</button>
        </div>
    </div>
</div>

</body>
</html>
