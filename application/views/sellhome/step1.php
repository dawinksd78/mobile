...
if($seletedCategory != '')
{
    ...
    if($seletedCategory == 'OFT' || $seletedCategory == 'ONE') {
        $ONE_ROOM_TYPE = $step_1['ROOM_TYPE'];
    }
    
    if($seletedCategory == 'APT' || $seletedCategory == 'OFT')
    {
        $SIDO_CODE = $step_1['SIDO_CODE'];
        $SIGUNGU_CODE = $step_1['SIGUNGU_CODE'];
        $DONG_CODE = $step_1['DONG_CODE'];
    ...
    <section id="container">
        <div class="sub_container">            
            <div class="cont_wrap join_wrap sellitm">
                ...
                <div class="proc">
                	<a href="" class="bul_proc on"></a><a href="" class="bul_proc"></a><a href="" class="bul_proc"></a><a href="" class="bul_proc"></a>
                </div>
                ...
                <div class="cont" id="infoSelectArea">
                    ...
                    <div class="inpbox" id="insertInfoArea1">
                        <div class="radio_box">
                            <div class="radio">
                                <label for="relat01">
                                    <input type="radio" name="REG_TYPE" id="REG_TYPE01" value="1" onclick="ownertenantcheck('1')" <?php if( !isset($REG_TYPE) || $REG_TYPE == '1') echo 'checked=""'; ?>>
                                    <i></i><strong><label for="REG_TYPE01">본인(집주인)</label></strong>
                                </label>
                            </div>
                            <div class="radio">
                                <label for="relat02">
                                    <input type="radio" name="REG_TYPE" id="REG_TYPE02" value="2" onclick="ownertenantcheck('2')" <?php if( isset($REG_TYPE) && $REG_TYPE == '2') echo 'checked=""'; ?>>
                                    <i></i><strong><label for="REG_TYPE02">세입자</label></strong>
                                </label>
                            </div>
                        </div>
                        <p class="add_text">* 본인 또는 세입자 이외의 대리인(법정대리인 포함)은 집내놓기 신청이 불가합니다.</p>
                    </div>
                    
                    <div class="inpbox" id="ownerPhoneInput" style="display:<?php if( !isset($REG_TYPE) || $REG_TYPE == '1') echo "none"; ?>;">
                        <label for="host_phone" class="lbl">집주인연락처(세입자일 경우)</label>
                        <input type="tell" name="OWNER_CP" id="OWNER_CP" value="<?php echo ( (isset($REG_TYPE) && ($REG_TYPE == '2') && isset($OWNER_CP)) ) ? $OWNER_CP : ""; ?>" placeholder="집주인연락처입력" title="집주인연락처입력" class="inp" autocomplete="off">
                    ...
                    <div class="inpbox rdinp">
                        ...
                        <div class="radio_box02">
                            <div class="rd01 rd02">
                                <input type="radio" name="CATEGORY" id="CATEGORY01" onclick="cateSelFn('APT')" value="APT" <?php if( !isset($seletedCategory) || $seletedCategory=='APT') echo 'checked=""'; ?>>
                                <label for="CATEGORY01">아파트</label>
                            </div>
                            <div class="rd01 rd02">
                                <input type="radio" name="CATEGORY" id="CATEGORY02" onclick="cateSelFn('OFT')" value="OFT" <?php if( isset($seletedCategory) && $seletedCategory=='OFT') echo 'checked=""'; ?>>
                                <label for="CATEGORY02">오피스텔</label>
                            </div>
                            <div class="rd01 rd02">
                                <input type="radio" name="CATEGORY" id="CATEGORY03" onclick="cateSelFn('ONE')" value="ONE" <?php if( isset($seletedCategory) && $seletedCategory=='ONE') echo 'checked=""'; ?>>
            ...
            <div class="btn_area bot_btn">
                <button type="button" class="btn_type02" onclick="sellstep1Proc()">다음</button>
            </div>
		</div>
    </section>
...
function floorListPrint1(obj)
{
	...
	<?php if($seletedCategory != '') { ?>
		<?php if($seletedCategory == 'APT' || $seletedCategory == 'OFT') { ?>
			var dongNo = '<?php echo $step_1['dongNm']; ?>';
...
// 매물종류
<?php if($seletedCategory != '') { ?>
	cateTypeSel('<?php echo $seletedCategory; ?>', 'selval');
	tradeType('<?php echo $TRADE_TYPE; ?>');

	// 주소 출력
	<?php if($seletedCategory == 'APT' || $seletedCategory == 'OFT') { ?>
		gugunListPrint('<?php echo ($SIDO_CODE) ? $SIDO_CODE : $step_1['SIDO_CODE']; ?>');
		dongListPrint('<?php echo ($SIGUNGU_CODE) ? $SIGUNGU_CODE : $step_1['SIGUNGU_CODE']; ?>');
		dangiListPrint('<?php echo ($DONG_CODE) ? $DONG_CODE : $step_1['DONG_CODE']; ?>');
		dangiDongListPrint('<?php echo ($COMPLEX_IDX) ? $COMPLEX_IDX : $step_1['COMPLEX_IDX']; ?>');
		<?php if($step_1['dongNm'] != '') { ?>
    		floorListPrint1('<?php echo ($DONG) ? $DONG : $step_1['DONG']; ?>');
    		floorDongListPrint('<?php echo ($FLOOR) ? $FLOOR : $step_1['FLOOR']; ?>');
    		floorDongHoListPrint('<?php echo ($HO) ? $HO : $step_1['HO']; ?>');
    		$('#TOTAL_FLOOR_PREV').val('<?php echo $step_1['TOTAL_FLOOR']; ?>');
    		$('#TOTAL_FLOOR_NEW').val('<?php echo $step_1['TOTAL_FLOOR']; ?>');
    		$('#FLOOR_NEW').val('<?php echo $step_1['FLOOR']; ?>');
    		$('#HO_NEW').val('<?php echo $step_1['HO']; ?>');
		<?php } else { ?>
			// 동
			$('#DONG_NEW_INPUT_FORM').attr('disabled', false);
			$('#DONG_NEW').val('<?php echo ($DONG) ? $DONG : $step_1['DONG']; ?>');
		...
		$('#TOTAL_FLOOR_NEW').val('<?php echo $step_1['TOTAL_FLOOR']; ?>');
		$('#AREA2').val('<?php echo $AREA2; ?>');
		$('#AREA_SELECTED').val('<?php echo $step_1['AREA_SELECTED']; ?>');
	<?php } else if($seletedCategory == 'ONE') { ?>
	    $('#addr').val('<?php echo $step_1['LAW_ADDR1']; ?>');
	    $('#addrDetail').val('<?php echo $step_1['LAW_ADDR2']; ?>');
	    $('#TOTAL_FLOOR').val('<?php echo $step_1['TOTAL_FLOOR']; ?>');
	    $('#FLOOR').val('<?php echo $FLOOR; ?>');
	    $('#HO').val('<?php echo $HO; ?>');
	    $('#AREA2').val('<?php echo $AREA2; ?>');
	    $('#LAWDONG').val('<?php echo $step_1['LAW_DONG_CODE']; ?>');
	    $('#addr_LAT').val('<?php echo $step_1['LAT']; ?>');
	    $('#addr_LNG').val('<?php echo $step_1['LNG']; ?>');
	<?php } ?>
...
function dotnumber_only(evt)
{
    var charCode = (evt.which) ? evt.which : evt.keyCode;
    if(charCode == 190 || (charCode >= 37 && charCode <= 40)) {
    	return true;
    }
...
function sellstep1Proc()
{
	...
	// 전송할 데이터
 	var sendData = "&REG_TYPE=" + REG_TYPE + "&OWNER_CP=" + OWNER_CP + "&CATEGORY=" + CATEGORY + "&SIDO_CODE=" + SIDO_CODE  
