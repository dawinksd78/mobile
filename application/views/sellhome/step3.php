    ...
    <section id="container">
        <div class="sub_container">
                            	...
                            	<?php $p=6; foreach($inimage as $row) { ?>
                                    <li class="addeditem userimg">
                                    	<div class="btn_file_wrap">
                                       		<span class="add_pic"><img src="<?php echo $row['IMG_FULL_PATH']?>"></span> <a onClick="removeImg(this)" data-picname="up1" data-numbers="<?php echo $p; ?>" data-imguuid="<?php echo $row['IMG_IDX']?>" class="btn_del01"></a>
                                       	</div>
                                    </li>
                              	<?php $p--; } ?>
                            	...
                            	foreach($outimage as $row)
                            	{
                            	    if($q == 0) {
                            			...
                            			<div class="btn_file_wrap addeditem userimg">
                                            ...
                                            <!-- 첨부된 파일위치 --> 
                                        	<span class="add_pic"><img src="<?php echo $row['IMG_FULL_PATH']?>"></span> <a onClick="removeImg(this)" data-picname="up" data-numbers="<?php echo $q; ?>" data-imguuid="<?php echo $row['IMG_IDX']?>" class="btn_del01"></a>
                                    	</div>
                                    	<?php
                                    } else {
                                        ?>
                            ...
                            <ul class="upload_pic" id="viewul1">
                                <?php $p=6; foreach($inimage as $row) {?>
                                    ...
                                    <?php } else { ?>
                                        <li class="addeditem userimg">
                                            <div class="btn_file_wrap">
                                                <span class="add_pic"><img src="<?php echo $row['IMG_FULL_PATH']?>" alt=""></span> <a onClick="removeImg(this)" data-picname="up1" data-numbers="<?php echo $p; ?>" data-imguuid="<?php echo $row['IMG_IDX']?>" class="btn_del01"></a>
                                            </div>
                                        </li>
                                    <?php } ?>
                                <?php $p--; } ?>
            ...
            <div class="btn_area bot_btn double">
                <button type="button" class="btn_type03" onclick="goPage('/sellhome/step2/<?php echo $CATEGORY; ?>');">이전</button>
                <button type="button" class="btn_type02" onclick="fnSave()">다음</button>
            </div>
        </div>
    </section>
</div>
...
var outerimageNowcnt = outerimagecnt;
<?php /* 아파트 & 오피스텔 */ if($CATEGORY == 'APT' || $CATEGORY == 'OFT') { ?>
$("document").ready( function() {changeDefault();});
<?php } ?>
...
function fnSave()
{
  	var param = "CATEGORY="+category+"&step=step_3";
  	$.ajax({
    	url: '/sellhome/saveStep3/',
    	type: 'POST',
    	data: param,
    	dataType: 'json',
...
$(document).ready(function (e) {
	...
	$('#fileupload1').on('change', function(){
	    ...
	    for(var x=0; x < ins; x++)
		{
			...
			$.ajax({
				url: '/sendfile/up/'+category+'/in', // point to server-side PHP script
	            ...
	            success: function (result) {
	        		if( result.code == 200 )
		        	{
	        			var inserthtml = '<li class="addeditem userimg"><div class="btn_file_wrap"><span class="add_pic"><img src="' + result.data +'"></span> <a onClick="removeImg(this)" data-picname="up1" data-numbers="' + itemInlimit + '" data-imguuid="' + result.newUuid + '" class="btn_del01"></a></div></li>';
	...
	$('#fileupload').on('change', function(){
		...
		for(var x=0; x < ins; x++)
		{
			...
			$.ajax({
				url: '/sendfile/up/'+category+'/out', // point to server-side PHP script
				dataType: 'json', // what to expect back from the PHP script
				cache: false,
				contentType: false,
				processData: false,
				...
				success: function (result) {
					if( result.code == 200 )
					{
						if(outerimagecnt == 0) {
							...
							var inserthtml = '<li class="addeditem userimg"><div class="btn_file_wrap"><span class="add_pic"><img src="' + result.data + '" alt=""></span> <a onClick="removeImg(this)" data-picname="up" data-numbers="' + outerimagecnt + '" data-imguuid="' + result.newUuid + '" class="btn_del01"></a></div></li>';
						}
